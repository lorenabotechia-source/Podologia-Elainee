...
# --- CREDENCIAIS DIRETAS (Sem usar Secrets) ---
google_secrets = {
    "connections": {
        "gsheets": {
...
# Injetando as credenciais no Streamlit
...
with st.form("ficha_completa_digital", clear_on_submit=True):
    ...
    data_nasc = col_id1.date_input("Data de Nascimento:", value=None, format="DD/MM/YYYY")
    ...
    lista_doencas = ["Diabetes", "Hipertensão", "Cardíaco", "Anidrose", "Bromidrose", "Pé Cavo", "Pé Plano", "Calosidade", "Onicomicose"]
    ...
    with col_canvas1:
        ...
        canvas_p = st_canvas(fill_color="rgba(255, 255, 255, 0)", stroke_width=2, stroke_color="black", background_color="white", height=150, key="canvas_p")
    with col_canvas2:
        st.write("Assinatura Elaine Souza:")
        canvas_e = st_canvas(fill_color="rgba(255, 255, 255, 0)", stroke_width=2, stroke_color="blue", background_color="white", height=150, key="canvas_e")

    if st.form_submit_button("SALVAR FICHA DIGITAL"):
        if nome:
            try:
                try: df_antigo = conn.read(worksheet="Sheet1")
                except: df_antigo = pd.DataFrame()
                novo = pd.DataFrame([{"Nome": nome, "Telefone": telefone, "Data": str(date.today()), "Assinatura": "Digitalizada"}])
